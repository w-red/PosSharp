# PosSharp Documentation

Official documentation for **PosSharp**, the platform-agnostic, reactive UPOS framework
for .NET.

## 📚 Table of Contents

### [Compliance Matrix](compliance.md)

- **[UposDeviceBase](https://github.com/w-red/PosSharp/wiki/PosSharp.Core.UposDeviceBase)**: The base implementation class that provides common UPOS logic and property management.

Detailed breakdown of supported UPOS common properties, methods, and events.

### [API Reference (Overview)](api_list.md)

Overview of key public classes and reactive streams. For a complete, auto-generated technical reference, see the **[Full API Reference (GitHub Wiki)](https://github.com/w-red/PosSharp/wiki)**.

### [POS for .NET Compatibility](pos_for_dotnet.md)

Mapping between Microsoft POS for .NET (Point of Service SDK) classes/concepts and PosSharp equivalents.

- **[IUposDevice](https://github.com/w-red/PosSharp/wiki/PosSharp.Abstractions.IUposDevice)**: The core interface for all UPOS devices in PosSharp.

### [OPOS (OLE for Retail POS) Mapping Table](opos.md)

Translation guide for developers moving from legacy OPOS (ActiveX/OCX) controls to PosSharp.

### [JavaPOS (JPOS) Mapping](jpos.md)

Mapping between JavaPOS standards and PosSharp equivalents for developers from the Java ecosystem.

---

## 🌐 Language Versions

- [English (Current)](index.md)
- [日本語 (Japanese)](index.jp.md)
