...
[ドキュメント目次](docs/index.jp.md) | [English Documentation](docs/index.md)
...
- **ロックフリーな状態管理**: `AtomicState<T>` (CASベース) を導入し、手動ロックを排除。高並列環境での信頼性が向上しました。
- **Fluent な機能取得 API**: `Get<T>` メソッドを `As<T>` (例: `AsInt()`, `AsString()`) へ刷新。より直感的で一貫性のある記述が可能になりました。
- **ミューテーションスコア 100%**: コアロジックにおいてミューテーションテスト 100% を達成。論理的な不備を徹底的に排除しました。
- **メディエータースナップショット**: 複数のプロパティを一括でアトミックに取得できるスナップショット機能を導入しました。
